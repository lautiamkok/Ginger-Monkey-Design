...
function get_menu($menu_name = 'menu_main') {
    ...
    $parents_ids = $parents ? array_column($parents, 'id') : [];
...
// Create tree menu.
function create_tree(array $list, array $parents, string $id_name) {
    $tree = array();
    foreach ($parents as $k => $parent){
        if(isset($list[$parent[$id_name]])){
            $parent['children'] = create_tree(
                $list,
                $list[$parent[$id_name]],
                $id_name
            );
