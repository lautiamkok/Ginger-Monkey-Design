...
// Abstract fields to be reuse.
function add_crb_asset_fields(
    $type = 'file',
    $options = []
) {
    $fields = [];
    $picks = array_merge([$type], $options);

    // Remove `file`, if `image` (or any others below) is set in $options.
    ...
    $store = [
       ...
       'title' => Field::make('text', 'title', __('Title'))
        ...
        'alt' => Field::make('text', 'alt', __('Alternative (Alt) Text'))
            ->set_help_text('Set an alt text alt to this asset. Meta ID: alt.')
            ->set_width(100),
